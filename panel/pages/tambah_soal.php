...
$sss= str_replace("'","\'",$_REQUEST['tanyasoal']);
	$sql0 = mysqli_query($GLOBALS["___mysqli_ston"], "update cbt_soal set XTanya = '$sss' where XKodeSoal = '$_REQUEST[soal]' and Urut = '$_REQUEST[nom]'");
...
<?php	
$sqltanya = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_paketsoal where XKodeSoal= '$_REQUEST[soal]' and XGuru = '$_COOKIE[beeuser]'");
...
<?php    $sqlsoal = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT MAX(XNomerSoal) as maksi FROM `cbt_soal` WHERE XKodeSoal = '$_REQUEST[soal]'");
...
<?php
$sqlpil = mysqli_query($GLOBALS["___mysqli_ston"], "select * from cbt_siswa group by XPilihan");
